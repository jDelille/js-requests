...
function toggleDark() {
    sayHelloButton.classList.add("toggle-dark")
}

sayHelloButton.addEventListener('mouseover', toggleDark)
...
function toggleLight() {
    if(sayHelloButton.classList.contains = 'toggle-dark') {
        sayHelloButton.classList.remove('toggle-dark')
    }
} 

sayHelloButton.addEventListener('mouseout', toggleLight);
...
const ohMy = () => {
    ...
    axios.get('http://localhost:3000/animals')
        .then(res => {
            ...
            const animalArr = res.data
            for(let i = 0; i < animalArr.length; i++) {
                let para = document.createElement('p')
                para.textContent = animalArr[i]
...
const repeatMyParam = () => {
    //YOUR CODE HERE
    axios.get(`http://localhost:3000/repeat/repeat-my-param`)
        .then(res => {
...
const getRequest = () => {
    axios.get('http://localhost:3000/query-test?myQuery=best-query-ever&secondQuery=monkey')
...
/*
    ...
    both queries are added to the console.
*/
...
document.querySelector('button').addEventListener('submit', createFood);
